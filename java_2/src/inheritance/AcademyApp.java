...
public class AcademyApp {
	public static void main(String[] args) {
		...
		for (AcademyPerson person : persons) {
			// 오버라이드 선언되지 않은 자식클래스의 메소드를 호출하기 위해서는 명시적 객체 형변환을 이용하여
			// 참조변수에 자식클래스의 객체를 일시적으로 저장하여 자식클래스의 메소드 호출
			...
			// 오버라이드 선언된 자식클래스의 메소드는 묵시적 객체 형변환에 의해 부모클래스의메소드를 호출하지 않고 자식클래스의 메소드 호출
