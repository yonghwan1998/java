...
//Oreilly 그룹에서 배포한 cos 라이브러리를 다운로드 받아 프로젝트에 빌드 처리
...
public class UploadServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// => 이클립스의 작업 디렉토리(WorkSpace)가 아닌 WAS 프로그램이 사용하는 웹디렉토리(WebApps)의 파일 시스템 경로 반환
		...
		// => MultipartRequest 객채를 생성하면 모든 전달파일을 제공받아 서버 디렉토리에 저장 - 자동 업로드 처리
		...
		// => policy : FileRenamePolicy 객체 전달 (생략시 전달파일로 기존 파일 덮어씌우기)
		...
		// MultipartRequest.getParameterValues(String name) 메소드 호출
