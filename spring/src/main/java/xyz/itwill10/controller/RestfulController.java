...
public class RestfulController {
	 ...
	 * 저장하여 문자열로 클라이언트에게 응답되도록 처리 //문제점)리스폰즈 메세지 몸체부에는 Java 객체를 저장하여 응답 처리 불가능 - 406
	 * 에러코드 발생 //해결법)요청 처리 메소드에 의해 반환되는 Java 객체를 문자열(XML 또는 JSON)로 변환하여 //리스폰즈 메세지
	 * 몸체부에 저장하여 응답 처리 // => jackson-databind 라이브러리를 프로젝트에 빌드 처리하면 요청 처리 메소드에 의해
	 ...
	 * Object 객체의 JSON 형식의 문자열로 변환되어 응답 처리 return
