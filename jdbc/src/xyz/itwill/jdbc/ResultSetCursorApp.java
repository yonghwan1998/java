...
public class ResultSetCursorApp {
	public static void main(String[] args) throws SQLException {
		 ...
		 * rs.absolute(2);
		 * 
		 * //ResultSet.updateXXX(String columnLabel, XXX value) : ResultSet 객체에서
		 * ResultSet //커서가 위치한 처리행의 컬럼값을 변경하는 메소드 // => XXX는 컬럼값을 변경하기 위해 전달받기 위한 Java
		 * 자료형 rs.updateString("name", "임걱정");
		 * 
		 * //ResultSet.updateRow() : ResultSet 객체에서 ResultSet 커서가 위치되어 변경된 //컬럼값이 저장된 행을
		 * 실제 테이블에 적용하여 변경 처리하는 메소드 rs.updateRow();
		 */

		/*
		 * rs.absolute(3);
		 * 
		 * //ResultSet.moveToInsertRow() : ResultSet 객체에서 ResultSet 커서가 위치한 다음행에 //새로운
		 * 행을 삽입하고 기존행은 다음행으로 차례대로 이동 처리하는 메소드 rs.moveToInsertRow();
		 * 
		 * //새롭게 삽입된 행의 컬럼값을 변경 rs.updateInt("no", 4000); rs.updateString("name",
		 * "일지매"); rs.updateString("phone", "010-6715-9081"); rs.updateString("address",
		 * "서울시 종로구"); rs.updateString("birthday", "2000-12-31");
		 * 
		 * //ResultSet.insertRow() : ResultSet 객체에서 삽입된 행을 실제 테이블에 적용하여 //삽입 처리하는 메소드
		 * rs.insertRow();
		 */
